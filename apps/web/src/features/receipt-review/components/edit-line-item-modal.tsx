...
import {
  type UpdateLineItemDraftInput,
  updateLineItemDraftInputSchema,
} from '@/features/receipt-review/api/update-line-item';
...
type EditLineItemModalProps = {
  ...
  currency: string;
  isSaving: boolean;
  onSave: (draft: UpdateLineItemDraftInput) => void;
  onCancel: () => void;
};
...
export function EditLineItemModal({
  ...
  const form = useForm({
    defaultValues: {
      name: initial.name,
      unitPriceCents: initial.unitPriceCents,
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="sm:max-w-100" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle className="screen-title">Edit item</DialogTitle>
        </DialogHeader>
        <form
          className="contents"
          onSubmit={(event) => {
